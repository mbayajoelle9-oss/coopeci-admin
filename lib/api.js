...
export const ReportAPI = {
  ...
  parBuckets: () => api.get('/reports/par-buckets'),
  liquidity: () => api.get('/reports/liquidity'),
...
export const AccountingAPI = {
  ...
  addBankAccount: (payload) => api.post('/accounting/bank-accounts', payload),
  updateBankAccount: (id, payload) => api.put(`/accounting/bank-accounts/${id}`, payload),
  confirmTransfer: (id) => api.post(`/accounting/transfer/${id}/confirm`),
  printBankStatement: () => api.get('/accounting/bank-statement/print', { responseType: 'blob' }),
  printStatement: (type) => api.get(`/accounting/statements/${type}/print`, { responseType: 'blob' }),
};
export const ShareCapitalAPI = {
  overview: () => api.get('/share-capital'),
  memberSummary: (memberId) => api.get(`/share-capital/member/${memberId}`),
  subscribe: (payload) => api.post('/share-capital/subscribe', payload),
  reimburse: (payload) => api.post('/share-capital/reimburse', payload),
  printCertificate: (id) => api.get(`/share-capital/${id}/certificate`, { responseType: 'blob' }),
};
...
export const GovernanceAPI = {
  auditLogs: (params) => api.get('/governance/audit-logs', { params }),
  exportAuditLogs: (params) => api.get('/governance/audit-logs/export', { params, responseType: 'blob' }),
  exportAuditLogsPdf: (params) => api.get('/governance/audit-logs/export-pdf', { params, responseType: 'blob' }),
  consolidation: () => api.get('/governance/consolidation'),
};
export const MembersAPI = {
  ...
  resetPassword: (id) => api.post(`/members/${id}/reset-password`),
  history: (id) => api.get(`/transactions/member/${id}`),
  fiche: (id) => api.get(`/members/${id}/fiche`, { responseType: 'blob' }),
};
...
export const CreditAPI = {
  ...
  create: (payload) => api.post('/credits/applications', payload),
  ...
  printContract: (id) => api.get(`/credits/${id}/contract/print`, { responseType: 'blob' }),
  byApplication: (appId) => api.get(`/credits/by-application/${appId}`),
  byMember: (memberId) => api.get(`/credits/member/${memberId}`),
};
...
export const CashOpsAPI = {
  list: (status) => api.get('/cash/sensitive-operations', { params: { status } }),
  create: (payload) => api.post('/cash/sensitive-operations', payload),
  approve: (id) => api.post(`/cash/sensitive-operations/${id}/approve`),
  reject: (id, reason) => api.post(`/cash/sensitive-operations/${id}/reject`, { reason }),
  dailyReport: (date) => api.get('/cash/daily-report', { params: { date }, responseType: 'blob' }),
};
export const CreditProductAPI = {
  list: (all) => api.get('/credit-products', { params: { all } }),
  create: (payload) => api.post('/credit-products', payload),
  update: (id, payload) => api.put(`/credit-products/${id}`, payload),
};
export const ReconciliationAPI = {
  import: (file) => { const fd = new FormData(); fd.append('file', file); return api.post('/accounting/reconciliation/import', fd, { headers: { 'Content-Type': 'multipart/form-data' } }); },
};
export const EmployeeAPI = {
  ...
  documents: (userId) => api.get(`/employees/${userId}/documents`),
  addDocument: (userId, docType, file) => { const fd = new FormData(); fd.append('docType', docType); fd.append('file', file); return api.post(`/employees/${userId}/documents`, fd, { headers: { 'Content-Type': 'multipart/form-data' } }); },
  removeDocument: (userId, docId) => api.delete(`/employees/${userId}/documents/${docId}`),
  fiche: (userId) => api.get(`/employees/${userId}/fiche`, { responseType: 'blob' }),
};
